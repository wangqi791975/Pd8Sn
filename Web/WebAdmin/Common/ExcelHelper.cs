...
namespace Com.Panduo.Web.Common
{
    public class ExcelHelper : IDisposable
    {
        ...
        public DataTable ExcelToDataTable(string sheetName, bool isFirstRowColumn, int firstRowNum = 0)
        {
            ...
            try
            {
                ...
                if (sheet != null)
                {
                    ...
                    if (isFirstRowColumn)
                    {
                        for (int i = firstRow.FirstCellNum; i < cellCount; ++i)
                        {
                            ...
                            if (cell != null)
                            {
                                ...
                                if (cellValue != null)// && !data.Columns.Contains(cellValue))
                                {
                    ...
                    else
                    {
                        for (int i = firstRow.FirstCellNum; i < cellCount; ++i)
                        {
                            var column = new DataColumn(string.Format("{0}{1}", "column", i));
                            data.Columns.Add(column);
                        }
